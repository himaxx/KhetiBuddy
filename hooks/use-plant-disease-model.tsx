...
export type PlantType = "apple" | "potato" | "tomato" | "grape" | "corn"

export function usePlantDiseaseModel(plantType: PlantType) {
  ...
  const loadModel = async () => {
    ...
    try {
      ...
      const modelURL = `/models/${plantType}/model.json`
      const metadataURL = `/models/${plantType}/metadata.json`

      console.log(`Loading ${plantType} model from:`, modelURL)

      // Load the model
      modelRef.current = await window.tmImage.load(modelURL, metadataURL)
      setIsModelLoaded(true)
      console.log(`${plantType} model loaded successfully`)
    } catch (err) {
      console.error(`Error loading ${plantType} model:`, err)
  ...
  }, [isModelLoaded, isLoading, plantType])
